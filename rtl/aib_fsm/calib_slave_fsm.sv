...
module calib_slave_fsm #(
    ...
    output logic                        i_m_power_on_reset, // Power on reset signal
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (next_state != current_state) begin
                delay_counter <= '0; // Reset counter on every state change

                // Handle channel counter increment
                case(current_state)
                    FSM_SETUP_REGS_WR_33C, FSM_PA_STEP4_WR, FSM_PA_STEP6_WR, FSM_PA_STEP8_WR_34C, 
                    FSM_PA_STEP10_WR, FSM_PA_STEP11_WR, FSM_PA_STEP12_WR, FSM_PA_STEP13_WR: begin
                ...
                // Handle primary output signals
                i_m_power_on_reset <= (next_state == FSM_POR_ASSERT || next_state == FSM_POR_WAIT2);
                ...
                if(current_state == FSM_WAKE_UP_WAIT && next_state == FSM_PA_POLL_LOCK_RD) begin
            ...
            // Increment delay counter during wait states
            if(current_state inside {FSM_POR_WAIT1, FSM_POR_WAIT2, FSM_POR_WAIT3, FSM_WAKE_UP_WAIT}) begin
    ...
    always_comb begin
        ...
        case (current_state)
            FSM_IDLE: next_state = FSM_POR_START;

            // Power-On Reset Sequence
            FSM_POR_START: next_state = FSM_POR_WAIT1;
            FSM_POR_WAIT1: begin
                if (delay_counter >= WAKEUP_DELAY_CYCLES) next_state = FSM_POR_ASSERT;
            end
            FSM_POR_ASSERT: next_state = FSM_POR_WAIT2;
            FSM_POR_WAIT2: begin
                if (delay_counter >= (2 * WAKEUP_DELAY_CYCLES)) next_state = FSM_POR_DEASSERT;
            end
            FSM_POR_DEASSERT: next_state = FSM_POR_WAIT3;
            FSM_POR_WAIT3: begin
                if (delay_counter >= (2 * WAKEUP_DELAY_CYCLES)) next_state = FSM_SETUP_REGS_WR_208;
            end
            ...
            FSM_SETUP_REGS_WR_208: begin
                ...
                if (!avmm_waitrequest_i) next_state = FSM_SETUP_REGS_WR_210;
            end
            ...
            FSM_SETUP_REGS_WR_210: begin
                ...
                if (!avmm_waitrequest_i) next_state = FSM_SETUP_REGS_WR_218;
            end
            ...
            FSM_SETUP_REGS_WR_218: begin
                ...
                if (!avmm_waitrequest_i) next_state = FSM_SETUP_REGS_WR_33C;
            end
            ...
            FSM_PA_POLL_LOCK_CHECK: begin
                if (channel_idx == TOTAL_CHNL_NUM-1) begin
                    ...
                    else next_state = FSM_PA_POLL_LOCK_RD;
                end else begin
