// This module implements a synthesizable FSM for the 'sl_phase_adjust_wrkarnd'
// task. It handles a complex sequence of polling for lock signals and performing
// multiple read-modify-write operations across 24 channels on the slave AIB interface.

module sl_phase_adjust_wrkarnd_fsm #(
    ...
    // Internal registers
    logic [4:0] i_s1; // Loop counter for 0 to 23
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (next_state == POLL_READ_SETUP)       addr_reg <= {i_s1, 11'h344};
            else if (next_state == S3_4_READ_SETUP)  addr_reg <= {i_s1, 11'h344};
            else if (next_state == S5_6_READ_SETUP)  addr_reg <= {i_s1, 11'h344};
            else if (next_state == S7_8_READ1_SETUP) addr_reg <= {i_s1, 11'h350};
            else if (next_state == S7_8_READ2_SETUP) addr_reg <= {i_s1, 11'h34C};
            else if (next_state == S9_10_READ_SETUP) addr_reg <= {i_s1, 11'h350};
            else if (next_state == S11_READ_SETUP)   addr_reg <= {i_s1, 11'h344};
            else if (next_state == S12_READ_SETUP)   addr_reg <= {i_s1, 11'h350};
            else if (next_state == S13_READ_SETUP)   addr_reg <= {i_s1, 11'h33C};
    ...
    always_comb begin
        ...
        case (current_state)
            ...
            POLL_LOOP_CHECK: next_state = (i_s1 == 23) ? POLL_DELAY : POLL_READ_SETUP;
            ...
            S3_4_LOOP_CHECK: next_state = (i_s1 == 23) ? S5_6_LOOP_START : S3_4_READ_SETUP;
            ...
            S5_6_LOOP_CHECK: next_state = (i_s1 == 23) ? S7_8_LOOP_START : S5_6_READ_SETUP;
            ...
            S7_8_LOOP_CHECK: next_state = (i_s1 == 23) ? S9_10_LOOP_START : S7_8_READ1_SETUP;
            ...
            S9_10_LOOP_CHECK: next_state = (i_s1 == 23) ? S11_LOOP_START : S9_10_READ_SETUP;
            ...
            S11_LOOP_CHECK: next_state = (i_s1 == 23) ? S12_LOOP_START : S11_READ_SETUP;
            ...
            S12_LOOP_CHECK: next_state = (i_s1 == 23) ? S13_LOOP_START : S12_READ_SETUP;
            
            // --- Step 13 Section ---
            S13_LOOP_START: next_state = S13_READ_SETUP;
            S13_READ_SETUP: begin
                transaction_start = 1'b1;
                transaction_addr = {i_s1, 11'h33C};
            ...
            S13_LOOP_CHECK: next_state = (i_s1 == 23) ? SEQUENCE_DONE : S13_READ_SETUP;
