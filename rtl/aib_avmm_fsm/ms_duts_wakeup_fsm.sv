...
// the DUTs wakeup sequence for the MASTER side. It asserts the necessary signals
// to bring the AIB adapter out of configuration and into an active state.

module ms_duts_wakeup_fsm #(
    ...
    // Outputs for Master Interface 1 (intf_m1)
    ...
    always_comb begin
        ...
        case (current_state)
            ...
            SET_CONF_DONE: begin
                m1_i_conf_done = 1'b1;
                m1_ns_mac_rdy  = {TOTAL_CHNL_NUM{1'b1}};
                next_state     = WAIT_1;
                counter_load   = 1'b1;
            end
            ...
            DEASSERT_ADAPTER_RESET: begin
                m1_i_conf_done     = 1'b1;
                m1_ns_mac_rdy      = {TOTAL_CHNL_NUM{1'b1}};
                m1_ns_adapter_rstn = {TOTAL_CHNL_NUM{1'b1}};
                next_state         = WAIT_2;
                counter_load       = 1'b1;
            end
            ...
            SET_LOCK_REQ: begin
                ...
                m1_ns_mac_rdy             = {TOTAL_CHNL_NUM{1'b1}};
                m1_ns_adapter_rstn        = {TOTAL_CHNL_NUM{1'b1}};
                m1_ms_rx_dcc_dll_lock_req = {TOTAL_CHNL_NUM{1'b1}};
                m1_ms_tx_dcc_dll_lock_req = {TOTAL_CHNL_NUM{1'b1}};
